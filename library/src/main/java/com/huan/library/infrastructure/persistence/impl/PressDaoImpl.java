...
public class PressDaoImpl extends BaseDaoImpl<Press> implements PressDao {
	...
	public List<Press> selectPresses(final PressView pressView) {
		 ...
		 */
		if (pressView.getPressISBN() != null
				&& !"".equals(pressView.getPressISBN())) {
			whereSub.append(" and p.pressISBN like (:pressISBN) ");
		}
		if (pressView.getPressName() != null
				&& !"".equals(pressView.getPressName())) {
			whereSub.append(" and p.pressName like (:pressName) ");
		}
		if (pressView.getPressAddress() != null
				&& !"".equals(pressView.getPressAddress())) {
		...
		HibernateCallback callback_ = new HibernateCallback() {
			...
			public Object doInHibernate(Session session)
				...
				if (pressView.getPressISBN() != null
						&& !"".equals(pressView.getPressISBN())) {
					String temp = "%"
							+ pressView.getPressISBN().replace(" ", "%") + "%";
					query.setParameter("pressISBN", temp);
				}
				if (pressView.getPressName() != null
						&& !"".equals(pressView.getPressName())) {
					String temp = "%"
							+ pressView.getPressName().replace(" ", "%") + "%";
					query.setParameter("pressName", temp);
				}
				if (pressView.getPressAddress() != null
						&& !"".equals(pressView.getPressAddress())) {
					String temp = "%"
							+ pressView.getPressAddress().replace(" ", "%")
							+ "%";
	...
	public void insertPressBatch(List<Press> list) {
