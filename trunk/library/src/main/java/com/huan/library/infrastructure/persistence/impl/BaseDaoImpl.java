...
public class BaseDaoImpl<T> extends HibernateDaoSupportBean implements
	...
	public List<T> selectMaxTs() {
		...
		list = (List<T>) getHibernateTemplate().find(
				"from " + entityClass.getName());
		return list;
	}

	@SuppressWarnings("unchecked")
	public PageModel<T> selectByPage(final int pageNo, final int pageSize) {
					...
					public Object doInHibernate(Session session)
						...
						return session
								.createQuery(
										"from " + entityClass.getName()
												+ " order by id")
								.setFirstResult(pageNo).setMaxResults(pageSize)
								.list();
					}
	...
	public Long selectTotalRecords() {
		// return (Long)
		// this.getSession().createQuery("select count(*) from :entity")
