...
/**
 * 
 * @author shuaizhichun 2010-4-9
 */
@Repository("roleDao")
public class RoleDaoImpl extends BaseDaoImpl<Role> implements RoleDao {

	public List<Role> selectRoles(final RoleView view) {
		...
		HibernateCallback callback = new HibernateCallback() {
			...
			public Object doInHibernate(Session session)
				...
				// 判断是否分页
				if (view != null && view.getIsPage() && view.getStart() != null
						&& view.getLimit() != null) {
	...
	public Role selectById(Long roleId) {
		...
		role = (Role) this.getHibernateTemplate().find(hql, roleId).iterator()
				.next();
